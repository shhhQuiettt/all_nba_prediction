...
final_ml_dataset %>% group_by(PLAYER_NAME, YEAR) %>% summarise(count = n(), max_all_nba = max(VOTES_PERCENTAGE_ALL_NBA), max_all_rookie = max(VOTES_PERCENTAGE_ALL_ROOKIE)) %>% filter(count > 1) %>% arrange(desc(count))

final_ml_dataset %>% count()
final_ml_dataset = final_ml_dataset %>% distinct(PLAYER_NAME, YEAR, .keep_all = TRUE)
final_ml_dataset %>% count()
