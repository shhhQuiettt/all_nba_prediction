...
def build_ml_dataset(season_string, raw_dir):
    ...
    assert not advanced.empty, f"Advanced stats DataFrame for {season_string} is empty! (Note: Advanced stats begin in 1996-97)"
    ...
    # Filter Standings
    standings_subset = standings[['TeamID', 'Conference', 'PlayoffRank', 'WINS', 'LOSSES', 'WinPCT']].copy()
    standings_subset = standings_subset.rename(columns={'TeamID': 'TEAM_ID'})

    # 5. Clean Merge (Player + Standings)
    ...
    # Format ML features
    final_df['PlayoffRank'] = pd.to_numeric(final_df['PlayoffRank'], errors='coerce').astype(int)
    
    # Insert a Season column so you can distinguish years after the final merge
    final_df.insert(0, 'SEASON', season_string)

    # 6. Save to dataset/raw/
...
def main():
    # 7. Setup directory structure using pathlib
    ...
    # 8. Merge all validated seasons into one master dataset
    if all_seasons_data:
        ...
        logger.info(f"Pipeline Complete! Master dataset saved to: {master_file_path} with {len(master_df)} total rows.")
    else:
